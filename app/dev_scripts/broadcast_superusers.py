...
async def get_superuser_users() -> list[User]:
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(User)
            .where(User.role == ROLE_SUPERUSER, User.is_active.is_(True))
            .order_by(User.id)
        )
        return list(result.scalars().all())


async def broadcast_to_superusers() -> None:
    users = await get_superuser_users()

    if not users:
        print("Активные суперюзеры не найдены.")
        return

    print(f"Найдено суперюзеров: {len(users)}")
    print()
    ...
    confirm = (
        input("Отправить это сообщение всем суперюзерам? (yes/no): ").strip().lower()
    )
...
if __name__ == "__main__":
    asyncio.run(broadcast_to_superusers())
